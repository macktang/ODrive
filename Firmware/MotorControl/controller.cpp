...
bool Controller::anticogging_calibration(float pos_estimate, float vel_estimate) {
    if (anticogging_.calib_anticogging && anticogging_.cogging_map != NULL) {
        ...
        if (fabsf(pos_err) <= anticogging_.calib_pos_threshold &&
            ...
            if (anticogging_.dir_original) {
              anticogging_.cogging_map[anticogging_.index] = vel_integrator_current_ / 2.0f;
              anticogging_.index = anticogging_.index + 1;
            } else {
              anticogging_.cogging_map[anticogging_.index] += vel_integrator_current_ / 2.0f;
              anticogging_.index = anticogging_.index - 1;
            }
        ...
        if (anticogging_.dir_original) {
          if (anticogging_.index < axis_->encoder_.config_.cpr) {
            set_pos_setpoint(anticogging_.index, 0.0f, 0.0f);
            return false;
          } else {
            reached_code_1_ = 0.6f;
            anticogging_.dir_original = false; // go the other way now
          }
        } else {
          if (anticogging_.index >= 0) {
            reached_code_2_ += 0.0001f;
            set_pos_setpoint(anticogging_.index, 0.0f, 0.0f);
            return false;
          } else {
            reached_code_3_ += 0.0007f;
